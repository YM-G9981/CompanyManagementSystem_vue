<template>
    <el-row>
        ...
        <el-col :span="2" >
            <el-button type="primary" circle :disabled="disabled" @click="exportSigns()" v-if="selectState !=''">
                <el-icon>
                    <Folder/>
                </el-icon>
            </el-button>
...
data(){
    ...
    methods :{
        ...
        getSigns(){
            var path = "/getSignRecord"
            switch(this.selectState){
                case "department":{
                    path = path + "/getSignRecordByDepartmentId/" + this.value
                    break
                }
                case "position":{
                    path = path + "/getSignRecordByPositionId/" + this.value
                    break
                }
                case "personal":{
                    path = path + "/getSignRecordByPersonalId/"+ this.value
                    break
                }
                case "name":{
                    path = path +"/getSignRecordLikeName/"+ this.value
                    break
                }
                case "sex":
                    path = path + "/getSignRecordBySex/" + this.value
                    break
                case "state":
                    path = path + "/getSignRecordByState/" + this.value
                    break
            }
            path = path + "/" + this.date
            axios.post(path).then(
                (res)=>{
                    this.$.emit("update:signs",res.data)
                }
            )
        },
        exportSigns(){
            var path = "/exportSignRecord"
            switch(this.selectState){
                case "department":{
                    path = path + "/getSignRecordByDepartmentId/" + this.value
                    break
            ...
            axios.get(path,{responseType: 'blob',}).then(
                (res)=>{
                    fileDownload(res.data, '签到.xlsx');
                }
            )
        }
    },
